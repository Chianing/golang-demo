...
func GetFileType(fileData []byte) string {
	...
	fileDataPrefix := fileData[:10]
	var fileType string
	fileCode := bytesToHexString(fileDataPrefix)
...
// GetImageSize
// 获取图片像素
func GetImageSize(filePath string) (height, width int, err error) {
	...
	fileByteStream, err := ioutil.ReadAll(file)
	if err != nil {
		fmt.Println(err)
		return
	}
	fileType := GetFileType(fileByteStream)
	fmt.Println(fileType)
	reader := bytes.NewReader(fileByteStream)
	config, s, err := image.DecodeConfig(reader)
	if err != nil {
		fmt.Println(err)
		return
	}
